...
def probe(
    services: list[str], key: str, frm: str, to: str, bucket: str, sample: int = 200
) -> dict:
    ...
    n_sql = len(sql)
    calls = [
        (["query", "-q", q, *window_flags(frm, to), "--json"], "object") for q in sql
    ]
    ...
    from seq_cli import (
        _normalise_query,  # the query envelope, normalised like query() does
    )
    ...
    for row in table(levels):
        svc_entry(_key(row.get(key)))["levels"][_key(row.get("@Level"))] = (
            row.get("n") or 0
        )
    for row in table(roots):
        svc_entry(_key(row.get(key)))["root_operations"][
            _key(row.get("@MessageTemplate"))
        ] = row.get("n") or 0
    ...
    for row in table(identities):
        svc_entry(_key(row.get(key)))["resource_identities"].append(
            {a: row.get(f"@Resource.{a}") for a in IDENTITY}
            | {"events": row.get("n") or 0}
        )
    for row in table(envflags):
        ...
        for p in ENV_PROPS:
            if row.get(f"has({p})"):
                e["environment_properties"][p] = (
                    e["environment_properties"].get(p, 0) + n
                )
        ...
        [
            (
                [
                    "query",
                    "-q",
                    f"select distinct({p}) from stream{w}",
                    *window_flags(frm, to),
                    "--json",
                ],
                "object",
    ...
    report["environment_values"] = {
        p: sorted(
            str(v)
            for v in (row[0] for row in (r.data or {}).get("rows") or [])
            if v is not None
        )[:20]
    ...
    if mdefs.ok and isinstance(mdefs.data, dict):
        ...
        report["metric_definitions"] = [
            {"name": d.get("Name"), "kind": d.get("Kind"), "unit": d.get("Unit")}
            for d in defs
        ]
    for e in per.values():
        e["root_operations"] = dict(
            sorted(e["root_operations"].items(), key=lambda kv: -kv[1])[:15]
        )
        e["levels"] = dict(sorted(e["levels"].items(), key=lambda kv: -kv[1]))
    for s in services:
        svc_entry(s)
    report["services"] = dict(
        sorted(per.items(), key=lambda kv: -(kv[1]["logs"] + kv[1]["spans"]))
    )
    report["data_slices"] = [
        {"time": s["time"], "events": s.get("n") or 0}
        for s in slices(dist)
        if (s.get("n") or 0) > 0
    ]
    ...
    report["environment"] = {
        ...
        if not any(e["resource_identities"] for e in per.values())
        else "per service",
        "environment_properties": {
            p: sum(e["environment_properties"].get(p, 0) for e in per.values())
            for p in present
        },
        ...
        "read_from": (
            ...
            if any(e["resource_events"] for e in per.values())
                ...
                "the event properties " + ", ".join(present)
                if present
                else "nothing: no event carries a resource or an environment-naming property"
            )
        ),
    }
    report["failed"] = [
        {"command": r.command, "error": r.error} for r in results if not r.ok
    ]
...
def render(o: dict) -> str:
    ...
    sig = o["signals"]
    out.append(
        f"signals: logs={sig['logs']} spans={sig['traces']} metric points={sig['metrics']} profiles={sig['profiles']}"
    )
    if o["data_slices"]:
        out.append(
            "data sits in: "
            + ", ".join(f"{s['time'][11:16]}({s['events']})" for s in o["data_slices"])
        )
    else:
    ...
    out.append(
        ...
        + (
            "environment-naming properties: "
            + ", ".join(f"{p}={n}" for p, n in env["environment_properties"].items())
            if env["environment_properties"]
            else "environment-naming properties: none"
        )
    ...
    out.append(
        f"  property names on the newest {o['sampled_events']} events: "
        + ", ".join(f"{k}({n})" for k, n in o["sampled_keys"].items())
    )
    for name, e in o["services"].items():
        out.append(
            f"{name}: logs={e['logs']} spans={e['spans']} traces={e['traces']} exceptions={e['exceptions']} metric points={e['metric_points']}"
        )
        if e["levels"]:
            out.append(
                "  levels: " + "  ".join(f"{k}={v}" for k, v in e["levels"].items())
            )
        ...
        out.append(
            ...
            + (
                " env props: "
                + ", ".join(f"{p}={n}" for p, n in e["environment_properties"].items())
                if e["environment_properties"]
                else " env props: none"
            )
        )
        for ident in e["resource_identities"]:
            out.append(
                "  identity "
                + " ".join(f"{a}={ident.get(a)}" for a in IDENTITY)
                + f" events={ident['events']}"
            )
...
def main() -> int:
    ...
    ap.add_argument(
        "--bucket",
        default="1m",
        help="time slice for the data distribution (default 1m)",
    )
    ap.add_argument(
        "--sample",
        type=int,
        default=200,
        help="newest events whose property names are inventoried (default 200)",
    )
    add_window(ap)
    ns = ap.parse_args()
