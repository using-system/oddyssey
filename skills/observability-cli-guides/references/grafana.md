...
| Local oddyssey stack | `skills/setup-local-stack` (in this repo) | Carries a ready-made isolated `GCX_CONFIG` context (`admin`/`admin` against `http://localhost:3000`, datasource UIDs `prometheus`/`loki`/`tempo`/`pyroscope`). Use it instead of re-deriving context setup for the local stack; `gcx` is the mandatory query CLI. |
