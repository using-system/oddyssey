...
| Local oddyssey stack | `skills/gcx-local-stack` (in this repo) | Carries a ready-made isolated `GCX_CONFIG` context (`admin`/`admin` against `http://localhost:3000`, datasource UIDs `prometheus`/`loki`/`tempo`/`pyroscope`) plus a curl fallback for when `gcx` itself is unavailable. Use it instead of re-deriving context setup for the local stack. |
