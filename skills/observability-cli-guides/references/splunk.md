...
| Metrics (SPL, `mstats`) | `./splunk search 'search \| mstats avg(cpu.usage) WHERE index=metrics_idx span=1m'` | [mstats command](https://help.splunk.com/en/splunk-enterprise/search/spl-search-reference/10.4/search-commands/mstats) | Runs against dedicated metric indexes, not event indexes; if no `index=` is given it only searches the caller's *default* metrics indexes (returns nothing if none are set) — always pass `WHERE index=` explicitly or `index=*` for "search everything". Group with `by:`; time-bucket needs an explicit `span=`. |
