...
| --- | --- | --- | --- |
| Metrics | `pup metrics query --query=<v2-metric-query> --from=<t> --to=<t>` | [gcx-style ref: pup README examples](https://raw.githubusercontent.com/DataDog/pup/main/README.md) | v2 timeseries query, e.g. `avg:system.cpu.user{*}`, `sum:app.requests{env:prod} by {service}`. `--from`/`--to` accept `1h`/`30m`/`7d`, RFC3339, Unix ms, or `now`. |
