...
| `depth` | yes (new reports) | How far the mission went — the second axis next to `mode`: `quick` queries the signals the question touches, fetches one exemplar per operation, collapses sections 3 to 6 and records only what it measured in section 7; `full` runs the whole protocol. A verification or re-measure records the depth it ran at | `quick`, `full`; absent on reports predating the field, which ran the full protocol and read as `full` — except to `/odd-verify`, which replays such a baseline at `quick` unless told `full verify`, and says so before dispatch |
