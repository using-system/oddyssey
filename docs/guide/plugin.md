...
one-hop source, the mode to replay, and whether the code
...
| [`/odd-config`](../../.apm/prompts/odd-config.prompt.md) | Show the configured backend - stack, environment, targeted instance, connection proof - and guide a backend switch, an environment switch (`target <environment>`) or a value persisted per environment, custom stacks listed as switch targets; the display writes nothing | `backend-configuration` (`## Check` to display, `## Switch` when the user picks a backend, an environment or a value); `observability-cli-guides` (`builtin-stacks.md`); reads `.odd/observability-stacks/` |
