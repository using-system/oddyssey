...
| [`observability-cli-guides`](../../.apm/skills/observability-cli-guides/SKILL.md) | One reference per stack - query surface, configuration display, what to persist - plus the built-in stack list: the local stack, Grafana (gcx), Datadog (Pup), Dynatrace (dtctl), Azure Monitor (az), CloudWatch (aws); the reference contract every stack file follows, with the script that checks a file against it, and the `grafana-*` scripts that query a Grafana stack per signal (the reference names them; a mission runs them instead of composing gcx calls) | Routes the local-stack case to `setup-local-stack` |
