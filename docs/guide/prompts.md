...
- "p95 under 300ms" / "error rate must stay under 1%" - the thresholds,
  ...
  evidence for you to raise, drop, re-scope, or keep with the floor
  acknowledged - it never adjusts a target itself;
...
- the depth is the baseline report's `depth` field, never asked; a
  ...
  arguments forces it either way. A quick verify rules what its
  signals cover and counts the rest as `not ruled (quick)`;
