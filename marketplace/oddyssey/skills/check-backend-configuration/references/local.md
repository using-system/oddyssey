...
reset (the `setup-local-stack` skill's `references/otel-lgtm-env.md`
catalogs what the image accepts). Show the variable names always, and
their values too — the stored contract holds flat scalars and never
secrets — but a name alone is the right display for anything that reads
like a credential. A `stack_config.local` that is present and empty
(`{}`) means **not configured**: display "no container env persisted",
never an error, and a missing key is the same statement.
