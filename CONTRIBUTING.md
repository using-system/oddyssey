...
uvx ruff@0.16.4 check .apm/skills/*/scripts .claude/skills/*/scripts tests/skills
uvx ruff@0.16.4 format --check .apm/skills/*/scripts .claude/skills/*/scripts tests/skills
...
- **A separate reviewer agent checks the branch before the PR** —
  and before every push that updates it: a sub-agent with a fresh
  context reviews the whole diff against the issue, each round of
  fixes goes back to the same reviewer until it returns no finding,
  the PR body records the outcome, and the push, the PR and the merge
  each wait for the maintainer's explicit go. AGENTS.md's "A separate
  reviewer agent checks the branch before its PR" section is the rule.
