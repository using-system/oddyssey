...
- **That first-use install is noisy, not broken.** The first invocation of
  ...
  module. None of it is a failure; a caller capturing combined
  stdout+stderr, or pattern-matching stderr for "error"/"warning" as a
  health signal, will misread this first-use noise as one.
