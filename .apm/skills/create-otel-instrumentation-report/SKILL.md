...
- **Never write secrets into a report**: no tokens, credentials, or
  ...
  Refer to access material by variable or secret name only. The rule
  reaches the **verification protocol's checks**: a check whose query
  projects a credential-bearing field (a connection string, a key, a
  token, an auth header) or whose expected outcome is one is a leak
