...
| Thresholds (the pass/fail targets) | human; the agent cross-checks each against the service-side floors its investigation finds and hands an unattainable one back before persisting | a target is a product decision, not a measurement - but whether the service can structurally meet it (a fixed sleep, a rate limit, an injected error rate) is a fact the investigation already holds, and a target below its floor measures the wrong path |
