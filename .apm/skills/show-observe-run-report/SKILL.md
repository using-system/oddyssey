...
- **The persistence return value** — what `create-observe-run-report`
  ...
  recalled-baseline line, section 2's tables with its deltas or
  rulings, section 3's ranked table, the telemetry gaps and the open
...
- **A stored report the caller names** — no return value in hand:
  read the same sections of that file from disk — never the whole
  file — and its carrying commit from git
